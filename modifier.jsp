...
<head>
    ...
    <title>ZEHNI - Modifier Article</title>
    ...
    <div class="modifier-endroit-container">
        <div class="form-box">
            ...
            <c:if test="${not empty msg}">
                <div class="alert-message ${msgType == 'success' ? 'success' : 'error'}">
                    <c:out value="${msg}"/>
                </div>
            </c:if>
            
            <!-- Section de recherche (si aucun élément n'est sélectionné) -->
            <c:if test="${empty element}">
                <div class="search-section">
                    <h3>Rechercher l'article à modifier</h3>
                    <form action="Controller" method="GET">
                        <input type="hidden" name="myBtn" value="rechercherarticle"/>
            ...
            <!-- Formulaire de modification (si un élément est sélectionné) -->
            <c:if test="${not empty element}">
                <form action="Controller" method="POST" class="modification-form">
                    <input type="hidden" name="myBtn" value="modifier"/>
                    <input type="hidden" name="id" value="<c:out value='${element.id}'/>"/>
                    
                    <div class="form-header">
                        <h2>Modifier l'article #<c:out value="${element.id}"/></h2>
                    </div>
                    
                    <div class="form-group">
                        <label>Titre : <span class="required">*</span></label>
                        <input type="text" name="nom" 
                               value="<c:out value='${element.titre}'/>" 
                               required class="form-input"/>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group half-width">
                            <label>Date de publication : <span class="required">*</span></label>
                            <input type="date" name="datepub" 
                                   value="<c:out value='${element.datepub}'/>" 
                                   required class="form-input"/>
                        </div>
                        ...
                        <div class="form-group half-width">
                            ...
                            <input type="text" name="imageurl" 
                                   value="<c:out value='${element.imageurl}'/>" 
                                   required class="form-input"/>
                        </div>
                    </div>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="modifierArticle.jsp" class="btn-cancel">
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    if (!isValid) {
                        ...
                        window.scrollTo(0, 0);
                    } else if (!confirm('Êtes-vous sûr de vouloir modifier cet article ?')) {
                        e.preventDefault();
                    }
                });
                
                // Réinitialiser les styles d'erreur lors de la modification
